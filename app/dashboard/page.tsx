...
export default function DashboardHomePage() {
  ...
  const myBookings = mockBookings.filter(
    booking => booking.requesterId === currentUser.id || booking.providerId === currentUser.id
  );
      ...
      <div className="grid md:grid-cols-4 gap-4">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium text-gray-600">Skills Offered</CardTitle>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{myBookings.filter(b => b.status !== 'rejected').length}</div>
          </CardContent>
        </Card>
            ...
            {myBookings.slice(0, 3).map((booking) => (
              <div key={booking.id} className="flex items-start gap-4 p-4 bg-gray-50 rounded-lg">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-500 mt-2">{booking.createdAt}</p>
                </div>
              </div>
            ))}
            {myBookings.length === 0 && (
              <div className="text-center py-8 text-gray-500">
                ...
                <Link href="/dashboard/browse">
                  <Button variant="link" className="mt-2">Browse skills to get started</Button>
                </Link>
              </div>
            )}
          </div>
          ...
          <div className="space-y-3">
            {mySkills.map((skill) => (
              <div key={skill.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                ...
                <Link href={`/dashboard/myskills/${skill.id}`}>
